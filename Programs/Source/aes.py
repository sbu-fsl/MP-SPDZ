...
class EmbeddedInverter():
    ...
    (TODO: Is this really faster / worth doing vs. just using built in inversion method on GF(2^40) element? Probably?)
...
class AESCipher():
    ...
    def __init__(self, num_rounds, key, sbox=None):
        '''
        '''
        self.block_size = 4 # Nb = 4 words = 16 bytes = 128 bits
    ...
    def key_expansion(self, key: list[sgf2n]) -> list[sgf2n]:
        ...
        key_schedule = [sgf2n(0)] * ((self.block_size * BYTES_PER_WORD) * (self.num_rounds + 1))
        # key_schedule = Array((self.block_size * BYTES_PER_WORD) * (self.num_rounds + 1), sgf2n)
        ...
        for i in range(self.key_length, self.block_size * (self.num_rounds + 1)): 
...
if __name__ == "__main__":
    ...
    def test_embedding():
        ...
        print_ln("a=%s, b=%s, inv_b=%s", a, b, b_preimage) # a=0xf, b=0x8000

        # test case 2: (x^6+x = 0x42) <=> (0x40100420 = y^30+y^20+y^10+y^5) 
        a = cgf2n(0x42)
        b = apply_field_embedding(a)
        b_preimage = apply_inverse_field_embedding(b)
        print_ln("a=%s, b=%s, inv_b=%s", a, b, b_preimage) # a=0x42, b=0x40100420
        ...
        print_ln("c_embedded=%s", c_embedded)
...
# def sub_word(word):
#     '''
#     Applies S-Box to each of the 4 bytes in word

#     :param word: A list of 4 bytes
#     '''
#     pass

# def aes_cipher(plaintext_blk, num_rounds, round_keys):
#     '''
#     AES cipher as standardized in FIPS 197. 

#     :param plaintext_blk: a 128-bit block of plaintext expressed as a 4x4 array of bytes
#     :param num_rounds: number of rounds (Nr). AES-128 uses 10 rounds, AES-196 12 rounds, AES-256 14 rounds.
#     :round_keys: key schedule obtained by key expansion on some appropriately sized key
#     '''
#     pass

# def aes_inverse_cipher(ciphertext_blk, num_rounds, round_keys):
#     '''
#     AES inverse cipher as standardized in FIPS 197

#     :param ciphertext_blk: a 128-bit block of ciphertext expressed as a 4x4 array of bytes
#     :param num_rounds: number of rounds (Nr). AES-128 uses 10 rounds, AES-196 12 rounds, AES-256 14 rounds.
#     :round_keys: key schedule obtained by key expansion on some appropriately sized key
#     '''
#     pass

# def key_expansion(num_rounds, key):
#     '''
#     Generates the key schedule for the AES cipher according to FIPS 197.
#     '''
#     pass

# def aes_128_encrypt(plaintext_blk, key):
#     round_keys = key_expansion(10, key)
#     return aes_cipher(plaintext_blk, 10, round_keys)

# def aes_128_decrypt(ciphertext_blk, key):
#     round_keys = key_expansion(10, key)
#     return aes_inverse_cipher(ciphertext_blk, 10, round_keys)

# def aes_196_encrypt(plaintext_blk, key):
#     round_keys = key_expansion(12, key)
#     return aes_cipher(plaintext_blk, 12, round_keys)

# def aes_196_decrypt(ciphertext_blk, key):
#     round_keys = key_expansion(12, key)
#     return aes_inverse_cipher(ciphertext_blk, 12, round_keys)

# def aes_256_encrypt(plaintext_blk, key):
#     round_keys = key_expansion(14, key)
#     return aes_cipher(plaintext_blk, 14, round_keys)

# def aes_256_decrypt(ciphertext_blk, key):
#     round_keys = key_expansion(14, key)
#     return aes_inverse_cipher(ciphertext_blk, 14, round_keys)